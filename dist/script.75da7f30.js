...
},{}],"loader-style.css":[function(require,module,exports) {
...
var _loaderStyle = _interopRequireDefault(require("./loader-style.css"));
  ...
  fields.map(row => {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      if (row[i].name == "Button_text" && row[i].value) {
        ...
        if (row[i].value && row[i].value.toString().toLowerCase() === "true") {
        ...
        } else if (row[i].value && row[i].value.toString().toLowerCase() === "false") {
          ...
          if (row[i].styles) {
            ...
            itemDOM.innerHTML = `<span>${row[i].formattedValue || row[i].value || ""}</span>`;
          }
        }
...
(async function () {
  roots.forEach(async function (root) {
    root.innerHTML = `<div class="lds-ripple-wrapper"><div class="lds-ripple"><div></div><div></div></div></div><style>${_loaderStyle.default}</style>`;
...
},{"./api":"api/index.js","./et-templates-style.css":"et-templates-style.css","./loader-style.css":"loader-style.css"}],"../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
